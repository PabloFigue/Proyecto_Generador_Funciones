...
tabla_conversion_display: ;tabla donde se retorna el valor de la suma. PARA ANODO
    clrf PCLATH ;0 al PCLATH
    bsf PCLATH,2 ;Bit mas significativo donde se encuentra la tabla
    bsf PCLATH,1
    bsf PCLATH,0
...
restart_tmr0 macro TMR0_REG ;Macro de Reinicio del conteo del TMR0
    ...
    bcf ((INTCON) and 07Fh), 2 ;Se apaga la bandera de interrupci?n por Overflow del TMR0
    endm
...
isr: ;instrucciones de la interrupcion
    btfsc ((PIR1) and 07Fh), 1 ;Verificacion de la bandera de interrupcion del Timer2
    call inte_TMR2
    btfsc ((INTCON) and 07Fh), 2 ;Verificacion de la bandera de interrupcion del Timer0
    call inte_TMR0
    btfsc ((INTCON) and 07Fh), 0 ;Verificacion de la bandera de interrupcion del PUERTOB
    call inte_portb
...
inte_TMR2: ;Subrutina de interrupcion del TMR2
    ...
    bcf ((PIR1) and 07Fh), 1 ;Apagado de la bandera de interrupcion del TMR2
...
inte_portb: ;interrupcion en el puertoB
    ...
    banksel PORTA
    bcf ((INTCON) and 07Fh), 0 ;Se limpia la bandera de interrupcion del PORTB
...
config_tmr2:
    ...
    bsf ((PIE1) and 07Fh), 1 ; (((PIE1) and 07Fh), 1) 1 = Enables the Timer2 to PR2 match interrupt
    movlw 0
    movwf PR2 ;Valor de comparacion del TMR2

    Banksel T2CON ;Banco 0
    bsf ((T2CON) and 07Fh), 6 ;Postcaler default 10 (1111)
    bsf ((T2CON) and 07Fh), 5
    bsf ((T2CON) and 07Fh), 4
    bsf ((T2CON) and 07Fh), 3

    bsf ((T2CON) and 07Fh), 1 ;Prescaler 16 (1X)
    bsf ((T2CON) and 07Fh), 0

    bcf ((PIR1) and 07Fh), 1

    bsf ((T2CON) and 07Fh), 2 ;Encendido del conteo del TMR2
    return
...
config_tmr0:
    ...
    bcf ((OPTION_REG) and 07Fh), 3 ;Asignación del Preescaler en el TMR0 = 0
    bsf ((OPTION_REG) and 07Fh), 2
    bsf ((OPTION_REG) and 07Fh), 1
    bsf ((OPTION_REG) and 07Fh), 0 ;Prescaler de 1:256 por DEFAULT(111)
...
config_push:
    ...
    bcf ((INTCON) and 07Fh), 0 ;Se limpia la bandera ((INTCON) and 07Fh), 0
    return
...
config_inte: ;configuracion de las interrupciones
    banksel PORTA
    bsf ((INTCON) and 07Fh), 7 ;Habilitacion de las interrupciones globales INTCON REGISTER

    bsf ((INTCON) and 07Fh), 3 ;Habilitacion de interrupcion por cambio en el PORTB
    bcf ((INTCON) and 07Fh), 0 ;Apagar bandera de cambio en el PORTB.
    bsf ((INTCON) and 07Fh), 5 ;Habilitacion de la interrupcion por overflow del TMR0
    bcf ((INTCON) and 07Fh), 2 ;Apagar bandera de overflow del TMR0
    return
